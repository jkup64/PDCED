...
if __name__ == '__main__':
    ...
    for round in range(args.epochs):
        ...
        print("Round {}, lr: {:.6f}, {}".format(round, lr, idxs_users))
        ...
        if (round + 1) % args.test_freq == 0:
            ...
            print('Round {:3d}, Average loss {:.3f}, Test loss {:.3f}, Test accuracy: {:.2f}'.format(
                round, loss_avg, loss_test, acc_test))
            ...
            results.append(np.array([round, loss_avg, loss_test, acc_test, best_acc]))
            final_results = np.array(results)
            final_results = pd.DataFrame(final_results, columns=['epoch', 'loss_avg', 'loss_test', 'acc_test', 'best_acc'])
            final_results.to_csv(results_save_path, index=False)

        if (round + 1) % 50 == 0:
            best_save_path = os.path.join(base_dir, 'fed/best_{}.pt'.format(round + 1))
            model_save_path = os.path.join(base_dir, 'fed/model_{}.pt'.format(round + 1))
